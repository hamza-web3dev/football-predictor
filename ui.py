...
def compute_stats(names, pdf, season):
    previous_season = season - 1
    sel = pdf[(pdf['name'].isin(names)) & (pdf['season'] == previous_season)]

    # Fallback: si le joueur n'a pas de données la saison passée
    if len(sel) == 0:
...
def player_positions(d, m, a, side):
    if side == 'home':
        xs = {'GK': 0.05, 'DEF': 0.21, 'MID': 0.45, 'ATT': 0.67}
    else:
        xs = {'GK': 0.95, 'DEF': 0.79, 'MID': 0.55, 'ATT': 0.33}
    pos = [(xs['GK'], 0.5, 'GK')]
...
def draw_pitch(hf, af, hp, ap, hn, an):
    ...
    # =========================
    for i in range(14):
        c = pitch_dark if i % 2 == 0 else pitch_light
        ax.add_patch(
            patches.Rectangle(
                (i / 14, 0.02),
                1 / 14,
    ...
    # CORNER ARCS
    # =========================
    for cx, cy in [(0.02, 0.02), (0.02, 0.98), (0.98, 0.02), (0.98, 0.98)]:
        ...
        ax.plot(
            cx + r * np.cos(t2 + base_angle),
            cy + r * np.sin(t2 + base_angle),
            color=line,
            lw=1.5,
            alpha=0.7,
            zorder=3
    ...
    # =========================
    def draw_player(x, y, number, name, main, edge, label_bg):
...
def get_player_opts(club_name, season):
    if club_name == '— Select Club —':
        filtered = players_df  # show all if no club selected yet
    else:
        filtered = players_df[(players_df['current_club_name'] == club_name)
                              &
                              (players_df['season'] == season - 1) ]
    return ['— Select Player —'] + sorted(filtered['name'].tolist())
...
home_player_opts = get_player_opts(home_club, season)
away_player_opts = get_player_opts(away_club, season)
...
with lc1:
    ...
    p = st.selectbox("GK", home_player_opts, key="h_gk", label_visibility="collapsed")
    ...
    for i in range(hd):
        with cols[i]:
            p = st.selectbox(f"DEF {i+1}", home_player_opts, key=f"h_d_{i}")
            home_selected.append(p if p != '— Select Player —' else None)
    ...
    for i in range(hm):
        with cols[i]:
            p = st.selectbox(f"MID {i+1}", home_player_opts, key=f"h_m_{i}")
            home_selected.append(p if p != '— Select Player —' else None)
    ...
    for i in range(ha):
        with cols[i]:
            p = st.selectbox(f"ATT {i+1}", home_player_opts, key=f"h_a_{i}")
            home_selected.append(p if p != '— Select Player —' else None)
...
with lc2:
    ...
    st.markdown("🧤 *Goalkeeper*")
    p = st.selectbox("GK", away_player_opts, key="a_gk", label_visibility="collapsed")
    ...
    for i in range(ad):
        with cols[i]:
            p = st.selectbox(f"DEF {i+1}", away_player_opts, key=f"a_d_{i}")
            away_selected.append(p if p != '— Select Player —' else None)
    ...
    for i in range(am):
        with cols[i]:
            p = st.selectbox(f"MID {i+1}", away_player_opts, key=f"a_m_{i}")
            away_selected.append(p if p != '— Select Player —' else None)
    ...
    for i in range(aa):
        with cols[i]:
            p = st.selectbox(f"ATT {i+1}", away_player_opts, key=f"a_a_{i}")
            away_selected.append(p if p != '— Select Player —' else None)
...
hs  = compute_stats(h_names, players_df, season)
as_ = compute_stats(a_names, players_df, season)
        ...
        with st.spinner("Running CatBoost model..."):
            try:
                ...
                if res.status_code == 200:
                    ...
                    pc2.metric("🟡 Draw",     f"{(probs['draw']*100)/1.5:.1f}%")
